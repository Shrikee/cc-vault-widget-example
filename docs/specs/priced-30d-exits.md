...
- **The price**: `requiredSpread` vectors — the runbook's pairs (`1.001000/1.000000 → 1000`,
  `2.000000/1.999999 → 1`), the live pair (`1.000396/1.000122 → 274`), a pair past the clamp →
...
- **The judgements**: `compareRequest` — the prototype's `request-above`, `request-below-stale`
  ...
  `request-fresh`, plus a marked-down-NAV vector → `above-share-price`. `largestPostableShares` — a mixed holding's exact boundary, strictly above its vested count (vested shares need zero spread, so some unvested tail always fits inside the 1%); one young lot → `null`; all vested → the balance. `floorSoundness` —
