...
// The vault overview's headline: the realised trailing APY over the headline
// (7-day) trailing window — the share price's growth, annualised linearly.
...
export function ApyHero({
  ...
  const apy =
    history.status === "ready" && sharePrice !== null
      ? computeWindowApy(history.events, sharePrice, now, HEADLINE_WINDOW)
      : null;
      ...
      <div className="apy-hero">
        <div>
          <div className="stat__label">
            {apy?.label ?? `${HEADLINE_WINDOW}d APY`}
          </div>
          <div className={`apy-hero__value${tone}`}>{value}</div>
        </div>
      </div>
      <p className="muted small apy-hero__hint">
        {failed
          ? HISTORY_UNAVAILABLE
          : `Last ${HEADLINE_WINDOW} days, annualised — not guaranteed.`}
      </p>
